const sdate = {
  ...
  timeago(ts: number, islong = false): string {
    ...
    if (interval >= 1) return `${interval} year(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 2592000)
    if (interval >= 1) return `${interval} month(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 604800)
    if (interval >= 1) return `${interval} week(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 86400)
    if (interval >= 1) return `${interval} day(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 3600)
    if (interval >= 1) return `${interval} hour(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 60)
    if (interval >= 1) return `${interval} minute(s)${islong ? " ago" : ""}`

    interval = Math.floor(seconds / 1)
    if (interval >= 1) return `${seconds} second(s)${islong ? " ago" : ""}`
